...
def plot_art_ms_improved():
    ...
    # Corregir el mapa de colores para que -500V sea rojo
    ...
    # Ajustar la constante para la aproximación armónica
    k_harmonic = abs(phi_min) / ((x_mm[-1]/2 - x_min)**2)
    phi_harmonic = phi_min + k_harmonic * (x_mm - x_min)**2

    # Recalcular posición del punto rojo
    x_min_idx = np.argmin(phi_axis)
    x_min = x_mm[x_min_idx]
    phi_min = phi_axis[x_min_idx]

    # Añadir punto rojo en el mínimo del potencial
